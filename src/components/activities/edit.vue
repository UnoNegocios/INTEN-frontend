<template>
    <v-card>
        ...
        <v-dialog v-model="createContactDialog" max-width="600px">
            <createContact v-bind:company="calendar.client_id" @closeCreateDialogContact="closeCreateDialogContact"/>
        </v-dialog>
        <!-- Crear empresa -->
...
import CreateCompany from "../clients/companies/create"
    ...
    created(){
        if(this.calendar.client!=undefined && this.calendar.client!=null && this.calendar.client!=''){
            this.entries.companies.push(this.calendar.client)
            this.clientType = 'empresa'
        }
        if(this.calendar.lead!=undefined && this.calendar.lead!=null && this.calendar.lead!=''){
            this.entries.leads.push(this.calendar.lead)
