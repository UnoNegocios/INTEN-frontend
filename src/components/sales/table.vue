    ...
    <v-container style="max-width:100vw;">
                ...
                <td :colspan="headers.length" class="pb-4">
                    <v-row class="pa-6 pb-0">
                        ...
                        <v-col cols="12" sm="6" md="1">
                            <v-btn v-if="item.pdf!=undefined" v-bind:href="process.env.VUE_APP_BACKEND + 'files/' + item.pdf" target="_blank" text style="margin:auto; display:block;">
                                PDF:
                                <br>
        ...
        <div class="text-center">
            <v-bottom-sheet  v-model="sheet2" inset>
                <v-sheet class="text-center" height="150px">
                    <div class="pt-6">
                    ¿Seguro que deseas cambiar el estatus de esta venta a {{statusData}}?
                    </div>
...
export default {
    ...
    computed: {
        ...
        quotationsList(){
            return this.$store.state.quotation.quotations.filter(quotation=>quotation.status == 'vendido' || quotation.status == 'cancelled')
        },
        barSales(){
    ...
    methods: { 
        ...
        */
        paymentStatus(id, total, invoice_date, company_id, serie){
            if(this.debt(total, id)<1){
                return 'Pagada'
            }
            else if(this.expirationDays(invoice_date, company_id)>0){
                if(this.creditDays(company_id)==null||this.creditDays(company_id)==undefined||this.creditDays(company_id)==''||this.creditDays(company_id)==0){
                    return 'Pendiente de Pago'
                }else{
                    return 'Vencida'
                }
            }
            else if(serie=='Serie A'){
                return 'En Credito'
            }
        ...
        payments(id){
            var collections = this.$store.state.collection.collection_details
