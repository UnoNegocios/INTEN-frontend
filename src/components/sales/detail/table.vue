    ...
    methods: { 
        ...
        apiCall () {
            return new Promise((resolve, reject) => {
                ...
                axios.get(process.env.VUE_APP_BACKEND + "api/v1/sale_utilities?itemsPerPage=" + itemsPerPage).then(response => {//" + link + "page=" + page + "
                    items = this.mapSales(response.data.data)
                    total = response.data.meta.total
                    if (sortBy.length === 1 && sortDesc.length === 1) {
                        if(sortDesc[0]){
                            axios
                            .get(process.env.VUE_APP_BACKEND + "api/v1/sale_utilities?" + link + "page=" + page + "&sort=-" + sortBy[0] + "&itemsPerPage=" + itemsPerPage)
                            .then(response=>{
                                items = this.mapSales(response.data.data)
                                total = response.data.meta.total
                                resolve({
